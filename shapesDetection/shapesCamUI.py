...
cv.createTrackbar(crop_minH,window_detection_name,0 , 240, nothing)
cv.createTrackbar(crop_maxH,window_detection_name,0 , 240, nothing)

cv.createTrackbar(crop_minW,window_detection_name,0 , 320, nothing)
cv.createTrackbar(crop_maxW,window_detection_name,0 , 320, nothing)
...
cv.setTrackbarPos(crop_minH, window_detection_name, 51)
cv.setTrackbarPos(crop_maxH, window_detection_name, 168)
cv.setTrackbarPos(crop_minW, window_detection_name, 85)
cv.setTrackbarPos(crop_maxW, window_detection_name, 246)
cv.setTrackbarPos(low_H_name, window_detection_name , 102)
cv.setTrackbarPos(high_H_name, window_detection_name , 143)
cv.setTrackbarPos(low_S_name, window_detection_name , 31)
cv.setTrackbarPos(high_S_name, window_detection_name , 234)
cv.setTrackbarPos(low_V_name, window_detection_name , 57)
...
while True:
    ...
    cv.imshow('gray',imgHSV)
    cv.imshow('frame_threshold',frame_threshold)

    thrash = cv.GaussianBlur(frame_threshold,(9,9),0)
