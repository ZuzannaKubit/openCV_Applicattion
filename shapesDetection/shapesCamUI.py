...
root = tk.Tk()
root.title('Zmiana wartości zmiennej')

threshold_label = tk.Label(root, text=f'Threshold: {threshold}')
threshold_label.pack()

threshold_slider = tk.Scale(root, from_=0, to=255, orient=tk.HORIZONTAL, command=UpdateThreshold)
...
while True:
    ...
    imgGry = cv.cvtColor(img, cv.COLOR_RGB2HSV)

    cv.imshow('gray',imgGry[:, :, 2])

    _, thrash = cv.threshold(imgGry[:, :, 2], threshold,255, cv.CHAIN_APPROX_NONE)
    ...
    for contur in contours:
        ...
        cv.drawContours(img, [approx], 0, (0,100,0), 5)
        x = approx.ravel()[0]
        y = approx.ravel()[1]
        if len(approx) == 3:
